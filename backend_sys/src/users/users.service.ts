...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto): Promise<User> {
    ...
    if (existingCin) {
      throw new ConflictException(
        `User with CIN ${createUserDto.cin} already exists`,
      );
    }
    ...
    if (createUserDto.address_id) {
      ...
      if (!address) {
        throw new BadRequestException(
          `Address with ID ${createUserDto.address_id} not found`,
        );
      }
    }
  ...
  async update(id: number, updateUserDto: UpdateUserDto): Promise<User | null> {
    ...
    if (updateUserDto.cin && updateUserDto.cin !== user.cin) {
      ...
      if (existingCin) {
        throw new ConflictException(
          `User with CIN ${updateUserDto.cin} already exists`,
        );
      }
    }
    ...
    if (updateUserDto.address_id) {
      ...
      if (!address) {
        throw new BadRequestException(
          `Address with ID ${updateUserDto.address_id} not found`,
        );
      }
      user.address = address;
    }
