...
export class MigrationCreateAcademicInstitution1746327062657
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "academic_institution" ADD CONSTRAINT fk_institution_address FOREIGN KEY (address_id) REFERENCES addresses(id)`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "academic_institution" DROP CONSTRAINT "fk_institution_address"`,
    );
