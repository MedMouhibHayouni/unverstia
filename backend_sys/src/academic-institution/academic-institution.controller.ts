...
export class AcademicInstitutionController {
  constructor(
    private readonly academicInstitutionService: AcademicInstitutionService,
  ) {}
  ...
  async update(
    @Param('id') id: string,
    @Body() updateDto: UpdateAcademicInstitutionDto,
  ) {
    try {
      const institution = await this.academicInstitutionService.update(
        +id,
        updateDto,
      );
      if (!institution) {
